const generateProduct = (products = 1, prefix = 'Product') => {
  ...
  <marketplace xmlns="https://schemas.cdon.com/product/4.0/4.8.0/product">
  `;
  for (let i = 0; i < products; i++) {
    xml += `<product>
    <identity>
        <id>${prefix.toLowerCase() + '-' + (i + 1)}</id>
    </identity>
    <title>
        <default>${prefix + ' ' + (i + 1)}</default>
    </title>
    <description>
        <default>${prefix + ' ' + (i + 1)} description</default>
    </description>
    <category>
        <puzzles_pegged_puzzles />
    </category>
</product>
     `;
  }
...
const generatePrice = (products = 1, prefix = 'Product') => {
  ...
  let xml = `<?xml version="1.0" encoding="utf-8"?>
  <marketplace xmlns="https://schemas.cdon.com/product/4.0/4.8.0/price">
  `;
  for (let i = 0; i < products; i++) {
    xml += `<product>
            <id>${prefix.toLowerCase() + '-' + (i + 1)}</id>
            <se>
                <salePrice>${salesPrice}</salePrice>
                <originalPrice>${price}</originalPrice>
                <shippingCost>29.0</shippingCost>
                <vat>25</vat>
            </se>
        </product>
     `;
...
const generateMedia = (products = 1, prefix = 'Product') => {
  let xml = `<?xml version="1.0" encoding="utf-8"?>
  <marketplace xmlns="https://schemas.cdon.com/product/4.0/4.8.0/media">
  `;
  for (let i = 0; i < products; i++) {
    xml += `<product>
    <id>${prefix.toLowerCase() + '-' + (i + 1)}</id>
